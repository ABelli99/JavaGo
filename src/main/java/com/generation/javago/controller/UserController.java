...
public class UserController {
	...
	/*
	/**
	 * Prende un intero dalla URI e restituisce un singolo Cliente, o se non presente un eccezione
	 * @param id
	 * @return
	 *\/
	@GetMapping("/clienti/{id}")
	public ClientewOrdiniDTO getOne(@PathVariable int id) 
	{
		if(repo.findById(id).isEmpty())
			throw new NoSuchElementException("sei sotto effetto di droghe fra");
		return new ClientewOrdiniDTO(repo.findById(id).get());
	}
	
	/**
	 * Prende una stringa con username dalla URI e restituisce un cliente
	 * @param username
	 * @return
	 *\/
	@GetMapping("/clienti/{username}/username")
	public ClientewOrdiniDTO getOne(@PathVariable String username) 
	{
		if(repo.findByUsername(username)==null)
			throw new NoSuchElementException("sei sotto effetto di droghe fra, sto username non esiste");
		return new ClientewOrdiniDTO(repo.findByUsername(username));
	}
	
	/**
	 * Entra un ClienteDTO che verrà controllato, se supera i controlli verrà inserito nel database
	 * @param dto
	 * @return
	 *\/
	@PostMapping("/clienti")
	public ClienteDTO createOne(@RequestBody ClienteDTO dto) 
	{
		Cliente toInsert = dto.revertToCliente();
		...
		return new ClienteDTO(repo.save(toInsert));
	}
